...
if __name__ == "__main__":
    ...
    lixeira_dir = '//workspaces//SchedulingAlgorithms//scripts//output//trash'
    arquivos = listar_arquivos(output_dir, '.csv')
    ...
    for benchmark, g in groupby(sorted(arquivos, key=lambda x: x['benchmark']), key=lambda x: x['benchmark']):
        ...
        benchmark_data.to_csv(f'{output_dir}//{benchmark}.csv', index=False, sep=';', decimal=',', float_format='%.2f')