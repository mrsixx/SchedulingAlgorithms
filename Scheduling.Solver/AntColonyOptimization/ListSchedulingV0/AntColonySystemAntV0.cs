...
namespace Scheduling.Solver.AntColonyOptimization.ListSchedulingV0
{
    ...
    public class AntColonySystemAntV0: BaseAnt<AntColonySystemAntV0>
    {
        ...
        {
            if (!Context.PheromoneStructure.TryGetValue(selectedMove, out double currentPheromoneValue) || !Context.PheromoneStructure.TryUpdate(selectedMove, (1 - Context.Phi) * currentPheromoneValue + Context.Phi * Context.Parameters.Tau0, currentPheromoneValue))
        ...
        private void LinkinToSink()
        {
            ...
            foreach (var sink in sinks)
            {
                ...
                var disjunction = sink.IncidentDisjunctions.Intersect(sinkDisjunctions).First(d => d.Machine.Index == machine);
